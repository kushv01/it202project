...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        try {
            ...
            if ($world_balance < $amount) {
                throw new Exception("World account does not have sufficient funds.");
            }

            $expected_world_balance = $world_balance - $amount;

            // Record deposit into user account
            $stmt = $db->prepare("
            ...
            // Record withdrawal from world account
            $stmt->execute([
                ":src" => $account_id,
            ...
            $db->commit();
            $success_message = "Deposit of $amount successful.";
        } catch (Exception $e) {
            $db->rollBack();
...
<head>
    ...
    <title>Deposit - VaultForge</title>
    <style>
        body {
